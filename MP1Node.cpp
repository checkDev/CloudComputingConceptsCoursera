...
bool MP1Node::recvCallBack(void *env, char *data, int size ) {
    ...
    switch(msgHdr->msgType)
    {
        ...
        case JOINREQ:
        {
            ...
            UpdateorAddHeartBeat(&a, *heartbeat);
...
bool MP1Node::UpdateorAddHeartBeat(Address *addr, long heartbeat , int hb ) {
    if (!hasGotMember(addr, heartbeat)) {
...
bool MP1Node::hasGotMember(Address *joinaddr, long heartbeat) {
    for (auto & member : memberNode->memberList) {
        if (member.getid() == getId(joinaddr) && member.getport() == getPort(joinaddr)) {
            if (member.heartbeat < heartbeat) {
...
Address MP1Node::getAddress(int address, short port) {
    ...
    Address addrs;
    memset(&addrs, 0, sizeof(Address));
    *(int *)(&addrs.addr) = address;
    *(short *)(&addrs.addr[4]) = port;
